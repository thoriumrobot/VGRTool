...
/** Refactoring to add nullability checks to fields and ensure safe access throughout the code. */
public class NullabilityRefactoring extends Refactoring {
  ...
  private boolean isNullCheck(Expression expr, String fieldName) {
    // Check if the expression is a null check of the form 'fieldName != null' or 'fieldName ==
    // null'
  ...
  private Expression createFieldExpression(ASTNode node, AST ast) {
    if (node instanceof FieldAccess) {
      ...
      newFieldAccess.setExpression(
          (Expression) ASTNode.copySubtree(ast, fieldAccess.getExpression()));
          ...
          public boolean visit(ReturnStatement returnStmt) {
            ...
            if (isNullLiteral(expr)) {
              ...
              Statement newStatement =
                  createReplacementStatement(ast, returnTypeBinding, returnStmt);
  ...
  private Statement createReplacementStatement(
      AST ast, ITypeBinding returnTypeBinding, ReturnStatement originalReturnStmt) {
